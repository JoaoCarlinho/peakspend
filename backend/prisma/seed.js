...
function randomAmount(avg, variance) {
...
function generateExpenses(userId, categoryMap, count) {
    const expenses = [];
    const notes = [
        'Business expense',
        'Weekly shopping',
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (!merchant)
            continue; // Skip if merchant is undefined
...
function generateTrainingData(userId, expenses, categoryMap) {
    const trainingData = [];
...
function generateMlModels(userId) {
    const now = new Date();
    const models = [
        {
            userId,
...
async function main() {
    ...
    for (const userData of USERS) {
        ...
        for (const record of trainingDataRecords) {
            ...
            if (!randomExpense)
                continue; // Skip if no expense found
