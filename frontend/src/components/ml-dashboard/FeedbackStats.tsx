...
export function FeedbackStats({ improvementMetrics, categoryBreakdown = [] }: FeedbackStatsProps) {
  ...
  const acceptanceRate = categoryBreakdown.length > 0
    ? categoryBreakdown.reduce((sum, cat) => sum + cat.accuracy * cat.predictions, 0) /
      categoryBreakdown.reduce((sum, cat) => sum + cat.predictions, 0)
    : 0.85; // Default estimate

  const correctionRate = 1 - acceptanceRate;

  // Pie chart data
  const pieData = [
    { name: 'Accepted', value: acceptanceRate * 100, color: '#4caf50' },
    { name: 'Corrected', value: correctionRate * 100, color: '#ff9800' },
  ];
        ...
        <Box sx={{ mt: 3, p: 2, bgcolor: 'grey.100', borderRadius: 2 }}>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={6}>
              ...
              <Typography
                variant="h6"
                color={accuracyChange7Days >= 0 ? 'success.main' : 'error.main'}
              >
            ...
            <Grid item xs={6}>
              ...
              <Typography
                variant="h6"
                color={accuracyChange30Days >= 0 ? 'success.main' : 'error.main'}
              >
